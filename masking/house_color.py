...
## uploading model
def upload_model(image:np.ndarray):
    '''Uploads the model obtained from "https://github.com/facebookresearch/segment-anything" and generate the predictor 
     and masks for the given image
    * predictor: Process the image to produce an image embedding
    * masks: list with all posible mask combinations
    ...
    sam_checkpoint = "imagine-houses/sam_vit_h_4b8939.pth"
    model_type = "vit_h"
...
def masking(input_point: np.ndarray, input_label: np.ndarray, image: np.ndarray, predictor):
...
if __name__ == "__main__":
    image = cv2.imread('imagine-houses/masking/samples/casa.jpeg')
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

    predictor_test, masks_test = upload_model(image)

    input_point = np.array([[50,65],[50,100],[123,50]])
    input_label = np.array([1,0,1])

    masks_test = masking(input_point, input_label, image, predictor_test)
